...
exports.dayFirstAssignLP = dayFirstAssignLP;
...
/* ── Bước 1: Encode input ────────────────────────── */
function encodeInputArray(days, symbolMap, size = 31) {
 ...
 */
function generateOneArrangement(pos, ones, zeros, pnRemaining, lastZeros, fixedArray, current, params, daysInMonth, firstOnePos, dailyRest, targetRest) {
    const total = fixedArray.length;
    if (pos === total)
        return pnRemaining === 0 ? current : null;
    const fixed = fixedArray[pos];
    if (fixed !== 0) {
        return generateOneArrangement(pos + 1, ones, zeros, pnRemaining, 0, fixedArray, [...current, fixed], params, daysInMonth, firstOnePos, dailyRest, targetRest);
    }
    const options = [];
 ...
 */
function dayFirstAssignLP(lpCounts, fixedArrays, initGaps, daysInMonth, maxConsecutiveDays, fixedWorking, totalNonX, empPhepNam, pnStartFromDay) {
 ...
 */
function step1_generateArrangement(emp, daysInMonth, month, year, params, isAccountingDept, symbolMap, dailyRest, targetRest, paidDayTypes) {
...
/* ── processEmployee ────────────────────────────── */
function processEmployee(emp, daysInMonth, month, year, params, shift1, shift2, isAccountingDept, groupWorkHours, symbolMap, paidDayTypes, dailyRest, targetRest) {

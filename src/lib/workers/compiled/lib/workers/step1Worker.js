...
const { emps, daysInMonth, month, year, params, accountingIds, monthId, now, algo } = worker_threads_1.workerData;
const accountingSet = new Set(accountingIds);
const rows = [];
for (const emp of emps) {
    const empInput = { ...emp, workdays: emp._normalizedWorkdays };
    const arrangement = (0, distributionEngine_1.step1_generateArrangement)(empInput, daysInMonth, month, year, params, accountingSet.has(emp.departmentId ?? ''), algo);

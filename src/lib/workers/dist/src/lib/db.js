...
async function seedIfEmpty(conn) {
  ...
  `);
    ...
    INSERT INTO alloc_rules (id, month_id, group_code, group_name, name, param_key, param_value, default_param, specific_value, description, active, created_at) VALUES
      ...
      ('wk1', '${MID}', 'WORK_RULE', 'Quy tắc làm việc', 'Giới hạn ngày làm liên tục',      'max_consecutive_days',         6,    '6 ngày',                                    'Sau tối đa 6 Giới hạn ngày làm liên tục phải có ít nhất 1 ngày nghỉ. Dùng để kiểm tra constraint backtracking.', '', TRUE, '${now}'),

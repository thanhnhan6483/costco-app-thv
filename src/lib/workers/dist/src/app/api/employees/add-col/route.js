...
async function POST() {
    ...
    try {
        ...
        if (!names.includes('so_ngay_lam_cuoi_thang_truoc')) {
            ...
            log.push('so_ngay already exists');
        ...
        await conn.close();
        return server_1.NextResponse.json({ ok: true, log });
    }
    catch (e) {
        await conn.close();
        return server_1.NextResponse.json({ error: String(e), log }, { status: 500 });
    }
}

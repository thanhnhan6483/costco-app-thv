...
async function GET(req) {
    ...
    try {
        ...
        const activeRuleRows = await conn.all(`SELECT param_key AS paramKey FROM alloc_rules WHERE month_id = ? AND active = TRUE`, monthId);
        const activeKeys = new Set(activeRuleRows.map(r => r.paramKey));
        // Load department map
        const deptRows = await conn.all(`SELECT id, code, name FROM departments WHERE month_id = ?`, monthId);
        const deptMap = new Map(deptRows.map(d => [d.id, d]));
        // Load distribution_results gộp theo NV
        const rawRows = await conn.all(`SELECT dr.employee_id AS empId, e.code AS empCode, e.name AS empName,
       ...
       ORDER BY e.code, dr.day`, monthId);
        ...
        const deptEmps = new Map();
        for (const emp of emps) {
            const dept = deptMap.get(emp.deptId);
            if (!dept || skipCodes.has(dept.code.toUpperCase()))
                continue;
            const lpCount = emp.days.filter(d => d.dayType === 1).length;
            if (!deptEmps.has(emp.deptId))
                deptEmps.set(emp.deptId, []);

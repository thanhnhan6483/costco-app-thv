...
async function POST(req) {
    ...
    try {
        ...
        // Load employees + dept
        const empRows = await conn.all(`SELECT e.id AS empId, e.code AS empCode, e.department_id AS deptId, d.code AS deptCode
       FROM employees e JOIN departments d ON e.department_id = d.id
       WHERE e.month_id = ?`, monthId);
        // Load distribution_results
        const drRows = await conn.all(`SELECT employee_id AS empId, day, day_type AS dayType
       FROM distribution_results WHERE month_id = ? ORDER BY employee_id, day`, monthId);
        ...
        for (const r of drRows) {
            ...
            if (arr && r.day >= 1 && r.day <= daysInMonth)
                arr[r.day - 1] = r.dayType;
        }
        // Group by dept (skip excluded depts)
        const deptGroups = new Map(); // deptId → empIds
        for (const e of empRows) {
            if (skipCodes.has(e.deptCode.toUpperCase()))
                continue;
            if (!deptGroups.has(e.deptId))
                deptGroups.set(e.deptId, []);
            ...
            for (let mi = 0; mi < members.length; mi++) {
                ...
                if (currentLP > target) {
                    ...
                    for (let i = 0; i < daysInMonth && toRemove > 0; i++) {
                        ...
                        let runLen = 1;
                        for (let j = i - 1; j >= 0 && arr[j] === 0; j--)
                            runLen++;
                        for (let j = i + 1; j < daysInMonth && arr[j] === 0; j++)
                            runLen++;
                        if (runLen > params.maxConsecutiveDays)
                            continue;
                        arr[i] = 0;
        ...
        if (changes.length === 0) {
            await conn.close();
            return server_1.NextResponse.json({ ok: true, fixed: 0, message: 'Không có vi phạm cân bằng LP nào cần sửa' });
        }
        // Batch update

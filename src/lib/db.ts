...
// Promise-based mutex để tránh nhiều request cùng init DuckDB đồng thời
let dbInitPromise: Promise<Database> | null = null;

async function getDb(): Promise<Database> {
  if (globalThis.__duckdb) return globalThis.__duckdb;
  ...
  dbInitPromise = (async () => {
    ...
    try {
      ...
      await conn.close();
      globalThis.__duckdb = db;
      return db;
    } catch (err) {
      await db.close().catch(() => {});
      dbInitPromise = null; // cho phép retry nếu thất bại
      throw err;
    }
  })();
...
async function seedIfEmpty(conn: Connection): Promise<void> {
  ...
  await conn.run(`
    INSERT INTO alloc_rules (id, month_id, group_code, group_name, name, param_key, param_value, default_param, specific_value, description, active, created_at) VALUES
      ...
      ('ar_5',  '${MID}', 'SHIFT_BALANCING_RULE', 'Quy tắc phân bổ ca', 'Phân bổ ca cân bằng',                'max_shift_difference',           1,    'Chênh lệch ≤ 1 NV',  '', 'Số lượng nhân viên giữa các ca trong cùng phòng ban phải gần bằng nhau mỗi ngày.',                                        TRUE, '${now}'),

...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Lấy toàn bộ id
    const emps = await conn.all<{ id: string }>(`SELECT id FROM employees WHERE month_id = ? ORDER BY code`, mid);
    ...
    await conn.close();
    return NextResponse.json({ ok: true, total, updated: shuffled.length, monthId: mid });
  } catch (e) {
    await conn.close();

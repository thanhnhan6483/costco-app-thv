...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await conn.all(
      `SELECT id, month_id AS monthId, code, name, description, paid, note, created_at AS createdAt
...
export async function POST(req: NextRequest) {
  try {
    const { id, code, name, description, paid, note, createdAt, monthId } = await req.json();
    const mid = monthId ?? DEFAULT_MONTH_ID;
    const conn = await getConn();
    await conn.run(
      `INSERT INTO leave_types (id,month_id,code,name,description,paid,note,created_at) VALUES (?,?,?,?,?,?,?,?)`,
      id, mid, code.toUpperCase(), name, description??'', paid??true, note??'', createdAt
    );
    await conn.close();
